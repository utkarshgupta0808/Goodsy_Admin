...
public class ShopAdapter extends FirestoreRecyclerAdapter<ShopModel, ShopAdapter.MyViewHolder> {
    ...
    protected void onBindViewHolder(@NonNull final ShopAdapter.MyViewHolder holder, final int position, @NonNull final ShopModel model) {
        ...
        holder.cardViewLayout.setOnClickListener(view -> {
            ...
            firebaseFirestore.collection("ShopKeeper").document(model.getShopKeeperId()).get().addOnSuccessListener(documentSnapshot -> {
                bundle.putString("ownerName", documentSnapshot.getString("ownerName"));
                bundle.putString("ownerPhone", documentSnapshot.getString("ownerNumber"));
                bundle.putString("ownerEmail", documentSnapshot.getString("ownerEmail"));
                bundle.putString("ownerAddress", documentSnapshot.getString("ownerCity"));
            ...
            }).addOnFailureListener(e -> Toast.makeText(holder.cardViewLayout.getContext(), e.getMessage(), Toast.LENGTH_SHORT).show());
        });
